          ...
          {faqs.map((faq, index) => (
            <Card key={index} className="overflow-hidden">
              ...
              >
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-medium text-brand pr-4 text-balance">
                      {faq.question}
                    </h3>
                    <svg
                      className={`w-5 h-5 text-brand transform transition-transform ${
              ...
              {openIndex === index && (
                <div
                  id={`faq-answer-${index}`}
                  className="px-6 pb-6 text-text-muted text-pretty"
                >
                  {faq.answer}
                </div>
              )}
